                ...
                {editingTodo && editingTodo.id === todo.id ? (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className={`flex-grow ${todo.completed ? 'text-gray-400' : 'text-gray-800'}`}>
                      <h4 className={`text-lg font-semibold ${todo.completed ? 'line-through' : ''}`}>{todo.title}</h4>
                      <p className="text-sm">{todo.description}</p>
                    </div>
                    <div className="hidden md:flex gap-1 flex-shrink-0">
                      <button onClick={() => handleStartEdit(todo)} className="p-2 text-gray-400 hover:text-indigo-600" title="Edit"><Pencil size={18} /></button>
                      <button onClick={() => handleDelete(todo.id)} className="p-2 text-gray-400 hover:text-red-600" title="Delete"><Trash2 size={18} /></button>
                    </div>
                    <div className="relative md:hidden flex-shrink-0">
                      <button onClick={() => setPopoverId(popoverId === todo.id ? null : todo.id)} className="p-2 text-gray-500 hover:text-gray-800">
                        <MoreVertical size={20} />
                      </button>
                      {popoverId === todo.id && (
                        <div ref={popoverRef} className="absolute right-0 top-full mt-2 w-32 bg-white rounded-md shadow-lg z-10 border">
                          <ul className="py-1">
                            <li><button onClick={() => handleStartEdit(todo)} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                              <Pencil size={16} /> Edit
                            </button></li>
                            <li><button onClick={() => handleDelete(todo.id)} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-600 hover:bg-gray-100">
